...
const d = await getDb();
await d.runAsync('INSERT OR REPLACE INTO items (id, uri, caption, createdAt, userId) VALUES (?,?,?,?,?)', [item.id, item.uri, item.caption ?? '', item.createdAt, item.userId]);
}
...
const d = await getDb();
const rows = await d.getAllAsync<GalleryItem>('SELECT * FROM items WHERE userId = ? ORDER BY createdAt DESC', [userId]);
return rows;
}
